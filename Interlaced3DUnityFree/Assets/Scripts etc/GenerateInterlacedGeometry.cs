...
public class GenerateInterlacedGeometry : MonoBehaviour 
{
    ...
    void CalculateCorners()
    {
        ...
        cornerNE = camRef.ScreenToWorldPoint(new Vector3((float)(Screen.width), (float)(Screen.height), GetComponent<Camera>().nearClipPlane + .0001f));
    ...
    void GenerateQuads()
    {
        ...
        for (int i = iStart; i < Screen.width; i+=2)
        {
            ...
            quads[index].GetComponent<Renderer>().material = depthMat;
        ...
        if (combineFilterGeometry) // this is optional. I left the original implementation as now you can easily see the additional performance boost
        {
            ...
            transform.GetComponent<Renderer>().material = depthMat;
        }
